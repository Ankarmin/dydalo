...
async function findCategory(slug: string): Promise<CatalogCategory | null> {
  if (isApiEnabled()) {
    try {
      const remote = await apiGetCategories();
      const match = remote.find((c) => c.slug === slug && c.active);
      if (match) return match;
    } catch {
      // Fallback local abajo.
    }
  }
  return categoriesStore.getBySlug(slug) ?? catalogCategories.find((c) => c.slug === slug) ?? null;
}
