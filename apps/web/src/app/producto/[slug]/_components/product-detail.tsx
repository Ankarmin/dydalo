...
import { Heart, Minus, Plus, ShoppingCart, ChevronRight } from "lucide-react";
...
export function ProductDetail({ product }: ProductDetailProps) {
  ...
  const displayCategory =
    categoryName.charAt(0) + categoryName.slice(1).toLowerCase();
    ...
    <div className="md:grid md:grid-cols-2 md:gap-10">
      ...
      <div className="mt-8 md:mt-0">
        <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <Link href={ROUTES.catalogo} className="transition-colors hover:text-foreground">
            Catálogo
          </Link>
          <ChevronRight className="size-3" />
          <Link href={ROUTES.catalogoCategory(product.category)} className="transition-colors hover:text-foreground">
            {displayCategory}
          </Link>
          <ChevronRight className="size-3" />
